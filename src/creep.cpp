...
Creep::Creep(Game &game, Pos pos) : Mob(game, pos) {
  ...
  ActivatorCollideBox hitbox(
      ...
      ActivatorCollideType::HIT_GOOD, 32 * SUBPIXELS_IN_PIXEL,
      0 * SUBPIXELS_IN_PIXEL, 32 * SUBPIXELS_IN_PIXEL, 0 * SUBPIXELS_IN_PIXEL);
  ...
  ReactorCollideBox hurtbox(
      ...
      hurtbox_offset_x, hurtbox_width, hurtbox_offset_y, hurtbox_height);
...
void Creep::attack() { switch_animation(2); }
...
void Creep::draw() {
  ...
  game.camera->render_ex(game.renderer, texture, NULL, &rect, 0, NULL,
                         SDL_FLIP_NONE);
}