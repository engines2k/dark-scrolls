...
void Text::draw() {
  ...
  texture = game.media.showFont(
      font, text,
      color); // SDL_CreateTextureFromSurface(game.renderer, surface);
...
void Incantation::draw() {
  ...
  if (index <= phrase.length()) {
    ...
    this->pos +=
        Translation{.x = SUBPIXELS_IN_PIXEL * 40,
                    .y = SUBPIXELS_IN_PIXEL *
                         -40}; /* must be modified later to scale with player*/
    ...
    untyped_texture =
        SDL_CreateTextureFromSurface(game.renderer, untyped_surface);
    SDL_QueryTexture(untyped_texture, NULL, NULL, &untyped_texW, &untyped_texH);
    undstrect = {pos.x + (typed_texW * SUBPIXELS_IN_PIXEL), pos.y, untyped_texW,
                 untyped_texH};
